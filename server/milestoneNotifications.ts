...
export async function notifyMilestoneAchievement(
  ...
  if (achievement.milestone.threshold >= 1000) {
    await notifyOwner({
      ...
      content: `${((distributor as any)?.displayName ?? "N/A") || distributor.username} just achieved ${achievement.milestone.name}! (${achievement.currentValue} ${achievement.milestone.category})`
    });
  }
